...
_SCATTERED_MIX_SORT_CACHE: Dict[Tuple[Any, ...], Tuple[EnhancedLibraryInfo, ...]] = {}
...
def _output_sequencing_mode_from_rule_code(rule_code: Any) -> str:
    """从规则矩阵读取最终输出用测序模式，未配置时回退到内部测序模式。"""
    code = _safe_str(rule_code, default="")
    if not code:
        return ""
    for profile in get_scheduling_config()._rule_matrix_config.get("lane_rule_profiles", []):
        if _safe_str(profile.get("rule_code"), default="") == code:
            return _safe_str(
                profile.get("output_sequencing_mode") or profile.get("sequencing_mode"),
                default="",
            )
    return _sequencing_mode_from_capacity_rule_code(code)


def _output_sequencing_mode_from_selection(selection: Any) -> str:
    """获取 LaneRuleSelection 对应的最终输出测序模式。"""
    profile = getattr(selection, "profile", None)
    if isinstance(profile, dict):
        output_mode = _safe_str(profile.get("output_sequencing_mode"), default="")
        if output_mode:
            return output_mode
    return _safe_str(getattr(selection, "sequencing_mode", ""), default="")
...
def _build_imbalance_fill_sort_key(
    ...
    *,
    ...
    if not current_libraries:
        ...
        return (0, 0 if resolved_candidate_is_imbalance else 1, 0.0, 0.0, -data)
    resolved_lane_summary = _resolve_lane_imbalance_summary(current_libraries, lane_summary)
    _, _, current_ratio = resolved_lane_summary
    projected_ratio = _project_lane_imbalance_ratio(
        current_libraries,
...
LANE_LOADING_HD_VISIUM_TYPES = {
    _normalize_text_for_match(item)
    for item in [
        "10x HD Visium空间转录组文库(新)",
    ]
}
    ...
    while len(lanes) < max_lanes:
        ...
        for combination_group, candidates in grouped.items():
            candidates = sorted(
                candidates,
                key=lambda item: (
                    _get_delivery_priority_sort_value(item),
...
def _keep_only_mode_1_1_prebuilt_lanes(
    lanes: List[LaneAssignment],
    remaining_libraries: List[EnhancedLibraryInfo],
    *,
    stage_label: str,
) -> Tuple[List[LaneAssignment], List[EnhancedLibraryInfo], Dict[str, float]]:
    """1.1阶段预抽取只保留真正1.1 Lane，降级成3.6T的文库回到1.1候选池。"""
    kept_lanes: List[LaneAssignment] = []
    returned_libraries = list(remaining_libraries or [])
    recycled_libraries = 0
    recycled_gb = 0.0
    recycled_lanes = 0

    for lane in list(lanes or []):
        if _get_lane_selected_mode(lane) == "1.1":
            kept_lanes.append(lane)
            continue
        recycled_lanes += 1
        for lib in list(getattr(lane, "libraries", []) or []):
            ...
            lib.lcxms = "1.1"
            returned_libraries.append(lib)
            recycled_libraries += 1
            recycled_gb += float(getattr(lib, "contract_data_raw", 0.0) or 0.0)

    if recycled_lanes:
        logger.info(
            "{}: 回收{}条非1.1预抽取Lane，返还{}个文库/{:.1f}G继续参与1.1 G53/G54/G55",
            stage_label,
            recycled_lanes,
            recycled_libraries,
            recycled_gb,
        )
    return kept_lanes, returned_libraries, {
        "recycled_lanes": recycled_lanes,
        "recycled_libraries": recycled_libraries,
        "recycled_gb": recycled_gb,
    }
...
def _parse_library_delivery_priority_time(lib: EnhancedLibraryInfo) -> Optional[float]:
...
def _get_delivery_priority_sort_value(lib: EnhancedLibraryInfo) -> float:
    """按wkdeliverydate排序，越临近/越早交付越优先；缺失值排最后。"""
    parsed = _parse_library_delivery_priority_time(lib)
    if parsed is None:
        return float("inf")
    return parsed
...
def _get_scattered_mix_delete_date_sort_value(lib: EnhancedLibraryInfo) -> float:
    """兼容旧调用名：实际使用wkdeliverydate作为非包Lane排机尝试优先级。"""
    return _get_delivery_priority_sort_value(lib)


def _get_delivery_priority_remove_sort_value(lib: EnhancedLibraryInfo) -> float:
    """修复/替换时优先让更晚交付的文库让位；缺失时间按最晚处理。"""
    value = _get_delivery_priority_sort_value(lib)
    if not math.isfinite(value):
        return float("-inf")
    return -value


def _get_libraries_delivery_priority_sort_key(
    libraries: Sequence[EnhancedLibraryInfo],
) -> Tuple[float, float, float]:
    """一组文库的交付时间排序键：越早交付的组合越靠前。"""
    values = sorted(
        value
        for value in (_get_delivery_priority_sort_value(lib) for lib in libraries or [])
        if math.isfinite(value)
    )
    if not values:
        return (float("inf"), float("inf"), float("inf"))
    return (values[0], values[len(values) // 2], sum(values) / len(values))
...
def _sort_remaining_for_scattered_mix_lane(
    ...
    """散样混排成Lane顺序：优先尝试wkdeliverydate更临近的文库。"""
    if not libraries:
        return libraries
    ...
    cache_key = tuple(
        sorted(
            (
                _get_compact_library_identity_value(lib),
                _get_delivery_priority_sort_value(lib),
            )
            for lib in libraries
        )
...
def _is_customer_library_candidate(lib: EnhancedLibraryInfo) -> bool:
    """识别客户文库：wksampleid/sample_id 第2-4位为 KDL。"""
    sample_id = _safe_library_text(lib, "sample_id", "wksampleid")
    return sample_id.upper()[1:4] == "KDL"
...
def _build_repaired_candidate_lane(
    *,
    ...
    max_fill_candidates: int = 80,
    max_replace_remove_candidates: int = 12,
    max_replace_add_candidates: int = 40,
    ...
    def _validate_items(
        ...
        if _time_budget_exhausted():
            return False, None, "time_budget_exhausted"
    ...
    def _try_drop_until_valid(
        ...
        *,
        ...
        for _ in range(min(max_removals, len(working))):
            ...
            removable.sort(
                key=lambda lib: (
                    _get_delivery_priority_remove_sort_value(lib),
                    _data(lib) if prefer_small else -_data(lib),
                    _safe_str(getattr(lib, "origrec", ""), default=""),
                )
            )
            removed = False
    ...
    def _is_10bp_repair_candidate(item: EnhancedLibraryInfo) -> bool:
        ten_bp_data = getattr(item, "ten_bp_data", None)
        if ten_bp_data is not None and ten_bp_data > 0:
            return True
        return bool(getattr(validator, "_is_10bp_index", lambda _: False)(getattr(item, "index_seq", "") or ""))

    def _is_ck_customer_repair_context() -> bool:
        real_selected = [item for item in selected if not _is_ai_balance_library(item)]
        if not real_selected:
            return False
        is_customer_only = all(
            _is_customer_library_candidate(item)
            for item in real_selected
        )
        if not is_customer_only:
            return False
        return lane_id.startswith("CK_") or _safe_str(
            metadata.get("selected_seq_mode") or metadata.get("seq_mode") or metadata.get("lcxms"),
            default="",
        ) == "3.6T-NEW"

    def _try_ck_customer_directed_repair(
        base_items: List[EnhancedLibraryInfo],
    ) -> Tuple[Optional[LaneAssignment], List[EnhancedLibraryInfo], str]:
        ...
        if not (
            ValidationRuleType.SPECIAL_LIBRARY_LIMIT in error_types
            or ValidationRuleType.BASE_IMBALANCE_RATIO in error_types
            or ValidationRuleType.INDEX_10BP_RATIO in error_types
        ):
            return None, [], ""

        min_allowed, max_allowed = _resolve_lane_capacity_limits(
            libraries=base_items,
            machine_type=machine_type,
            lane_id=lane_id,
            lane_metadata=metadata,
        )
        special_limit = _resolve_special_library_data_limit(machine_type)
        imbalance_ratio_limit = float(
            getattr(validator, "base_imbalance_ratio_limit_3_6t", CUSTOMER_MIX_IMBALANCE_MAX_RATIO)
            or CUSTOMER_MIX_IMBALANCE_MAX_RATIO
        )
        target_10bp_ratio = 0.40

        def _state(items: Sequence[EnhancedLibraryInfo]) -> Tuple[float, float, float, int, int]:
            total = _total(items)
            special_total = sum(_data(item) for item in items if _is_imbalance_library_candidate(item))
            tenbp_total = sum(_data(item) for item in items if _is_10bp_repair_candidate(item))
            return total, special_total, tenbp_total, len(list(items or [])), _count_lane_index_pairs(list(items or []))

        def _tenbp_shape_ok(items: Sequence[EnhancedLibraryInfo]) -> bool:
            real_items = [item for item in list(items or []) if not _is_ai_balance_library(item)]
            tenbp_total = sum(_data(item) for item in real_items if _is_10bp_repair_candidate(item))
            non10_total = sum(_data(item) for item in real_items if not _is_10bp_repair_candidate(item))
            if tenbp_total <= 0 or non10_total <= 0:
                return True
            return tenbp_total / (tenbp_total + non10_total) >= target_10bp_ratio - 1e-6

        def _metadata_for_tenbp_shape(items: Sequence[EnhancedLibraryInfo]) -> Dict[str, Any]:
            shaped_metadata = dict(metadata)
            real_items = [item for item in list(items or []) if not _is_ai_balance_library(item)]
            tenbp_total = sum(_data(item) for item in real_items if _is_10bp_repair_candidate(item))
            non10_total = sum(_data(item) for item in real_items if not _is_10bp_repair_candidate(item))
            if non10_total > 0 and tenbp_total <= 0:
                shaped_metadata["is_pure_non_10bp_lane"] = True
            elif tenbp_total > 0 and non10_total <= 0:
                shaped_metadata["is_pure_10bp_lane"] = True
            return shaped_metadata

        def _meets_light_targets(items: List[EnhancedLibraryInfo]) -> bool:
            total, special_total, tenbp_total, _, _ = _state(items)
            if total < min_allowed - 1e-6 or total > max_allowed + 1e-6:
                return False
            if special_total > special_limit + SPECIAL_LIBRARY_LIMIT_EPSILON:
                return False
            if total > 0 and special_total / total > imbalance_ratio_limit + 1e-6:
                return False
            if not _tenbp_shape_ok(items):
                return False
            if _violates_mode_1_1_add_test_cap(items, lane_metadata=metadata):
                return False
            return True

        def _append_compatible(
            working: List[EnhancedLibraryInfo],
            candidates: Sequence[EnhancedLibraryInfo],
        ) -> List[EnhancedLibraryInfo]:
            selected_idx_cache: List[Any] = [_parse_library_index_pairs_latest(item) for item in working]
            ...
            while remaining and not _meets_light_targets(working):
                ...
                def _candidate_key(item: EnhancedLibraryInfo) -> Tuple[Any, ...]:
                    data = _data(item)
                    is_10bp = _is_10bp_repair_candidate(item)
                    trial_total = total + data
                    trial_10bp = tenbp_total + (data if is_10bp else 0.0)
                    trial_ratio = trial_10bp / trial_total if trial_total > 0 else 0.0
                    trial_items = [*working, item]
                    needs_10bp = (
                        not _tenbp_shape_ok(working)
                        or (
                            not _tenbp_shape_ok(trial_items)
                            and trial_10bp > 0
                        )
                    )
                    return (
                        0 if (needs_10bp and is_10bp) or (not needs_10bp and not is_10bp) else 1,
                        max(0.0, target_10bp_ratio - trial_ratio),
                        abs(data - fill_gap) if fill_gap > 0 else data,
                        _get_delivery_priority_sort_value(item),
                        -_count_library_index_pairs(item),
                        _safe_str(getattr(item, "origrec", ""), default=""),
                    )
                ...
                for candidate in sorted(remaining, key=_candidate_key):
                    ...
                    if id(candidate) in selected_ids:
                        continue
                    if not _is_customer_library_candidate(candidate):
                        continue
                    if _shares_split_family_with_selected(working, candidate):
                        continue
                    trial = working + [candidate]
                    trial_total, trial_special, _, _, _ = _state(trial)
                    if trial_total > max_allowed + 1e-6:
                        continue
                    if trial_special > special_limit + SPECIAL_LIBRARY_LIMIT_EPSILON:
                        continue
        ...
        # 先处理不均衡上限：3.6T客户纯Lane按比例约束，超限时必须先裁不均衡。
        while True:
            total, special_total, tenbp_total, _, _ = _state(working)
            ratio_over_limit = total > 0 and special_total / total > imbalance_ratio_limit + 1e-6
            if special_total <= special_limit + SPECIAL_LIBRARY_LIMIT_EPSILON and not ratio_over_limit:
                break
            removable = [
                item for item in working
                if _is_imbalance_library_candidate(item)
            ]
            if not removable:
                break
            excess = max(
                0.0,
                special_total - special_limit,
                special_total - imbalance_ratio_limit * total,
            )
            removable.sort(
                key=lambda item: (
                    0 if _data(item) >= excess - 1e-6 else 1,
                    abs(_data(item) - excess),
                    0 if not _is_10bp_repair_candidate(item) else 1,
                    _get_delivery_priority_remove_sort_value(item),
                    _data(item),
                    _safe_str(getattr(item, "origrec", ""), default=""),
                )
            )
        ...
        pool_candidates = [
            ...
            if id(item) not in {id(selected_item) for selected_item in working}
            and _is_customer_library_candidate(item)
            and not _is_imbalance_library_candidate(item)
            and not _is_mode_1_1_add_test_limited_library(item)
        ]
        pool_candidates.sort(
            key=lambda item: (
                0 if _is_10bp_repair_candidate(item) else 1,
                _get_delivery_priority_sort_value(item),
                -_data(item),
                -_count_library_index_pairs(item),
                _safe_str(getattr(item, "origrec", ""), default=""),
        ...
        if not _meets_light_targets(working):
            ...
            wider_candidates.sort(
                key=lambda item: (
                    0 if _is_10bp_repair_candidate(item) else 1,
                    0 if not _is_mode_1_1_add_test_limited_library(item) else 1,
                    _get_delivery_priority_sort_value(item),
                    -_data(item),
                    _safe_str(getattr(item, "origrec", ""), default=""),
                )
        ...
        if valid and lane is not None:
            logger.info(
                ...
                len(working),
                _total(working),
                "ck_trim_special_fill_10bp_customer",
            )
    ...
    def _try_index_conflict_directed_repair(
        ...
        for _ in range(min(max_replace_remove_candidates, len(working))):
            removable = [
                ...
                if conflict_counts.get(_get_library_identity_key(lib), 0) > 0
            ]
            if not removable:
                break
            removable.sort(
    ...
    def _try_special_split_directed_repair(
        ...
        if ss_valid:
            return None, [], ""
        if ss_reason == "hd_visium_and_group_b_mixed":
            hd_items = [
                item for item in base_items
                if _library_sample_type_matches_rule(item, LANE_LOADING_HD_VISIUM_TYPES)
            ]
            group_b_items = [
                item for item in base_items
                if _library_sample_type_matches_rule(item, LANE_LOADING_COMBO_GROUP_B)
            ]
            if not hd_items or not group_b_items:
                return None, [], ""
            hd_data = _total(hd_items)
            group_b_data = _total(group_b_items)
            if _get_libraries_delivery_priority_sort_key(hd_items) < _get_libraries_delivery_priority_sort_key(group_b_items):
                remove_hd = False
            elif _get_libraries_delivery_priority_sort_key(group_b_items) < _get_libraries_delivery_priority_sort_key(hd_items):
                remove_hd = True
            else:
                remove_hd = hd_data <= group_b_data
            kept = [
                item for item in base_items
                if (
                    not _library_sample_type_matches_rule(item, LANE_LOADING_HD_VISIUM_TYPES)
                    if remove_hd
                    else not _library_sample_type_matches_rule(item, LANE_LOADING_COMBO_GROUP_B)
                )
            ]
            if not kept or len(kept) == len(base_items):
                return None, [], ""
            valid, lane, _ = _validate_items(kept)
            action = "trimmed_hd_visium" if remove_hd else "trimmed_group_b_for_hd_visium"
            if valid and lane is not None:
                return lane, kept, action
            lane, repaired, fill_action = _try_add_candidates_until_valid(
                kept,
                (
                    (lambda lib: not _library_sample_type_matches_rule(lib, LANE_LOADING_HD_VISIUM_TYPES))
                    if remove_hd
                    else (lambda lib: not _library_sample_type_matches_rule(lib, LANE_LOADING_COMBO_GROUP_B))
                ),
                action=f"{action}_filled",
            )
            if lane is not None:
                return lane, repaired, fill_action
            return None, [], ""
        if ss_reason != "group_a_and_group_b_mixed":
            return None, [], ""
        mode_data: Dict[str, float] = {"A": 0.0, "B": 0.0}
        mode_earliest_delivery: Dict[str, float] = {"A": float("inf"), "B": float("inf")}
        for item in base_items:
            mode = _classify_library_special_split_mode(item)
            if mode in mode_data:
                mode_data[mode] += _data(item)
                mode_earliest_delivery[mode] = min(
                    mode_earliest_delivery[mode],
                    _get_delivery_priority_sort_value(item),
                )
        if mode_earliest_delivery["A"] < mode_earliest_delivery["B"]:
            remove_mode = "B"
        elif mode_earliest_delivery["B"] < mode_earliest_delivery["A"]:
            remove_mode = "A"
        else:
    ...
    for _ in range(min(len(selected), 8)):
        ...
        for item in selected:
            ...
            if trial_total >= min_allowed - 1e-6:
                removable.append((
                    _get_delivery_priority_remove_sort_value(item),
                    abs(max_allowed - trial_total),
                    float(getattr(item, "contract_data_raw", 0.0) or 0.0),
                    item,
                ))
        if not removable:
            break
        removable.sort(key=lambda entry: (entry[0], entry[1], entry[2]))
        selected = [lib for lib in selected if lib is not removable[0][3]]
    ...
    while (
        ...
        if prioritize_scattered_mix:
            ...
            if deterministic_candidate_order == "budgeted_ratio":
                ...
                candidates = _sort_remaining_for_scattered_mix_lane(list(active_pool))
                if attempt_idx > 1:
                    offset = (attempt_idx - 1) % len(candidates)
        ...
        if lane_id_prefix == "CK" and mode_for_diagnostics == "1.1":
            ...
            selected_10bp_total = _total_lane_data(
                [
                    ...
                    if (
                        getattr(item, "ten_bp_data", None) is not None
                        and getattr(item, "ten_bp_data", None) > 0
                    )
                    or bool(getattr(validator, "_is_10bp_index", lambda _: False)(getattr(item, "index_seq", "") or ""))
                ]
            )
            selected_10bp_ratio = selected_10bp_total / selected_total if selected_total > 0 else 0.0
            if selected_10bp_ratio < 0.40 - 1e-6:
                ck_low_10bp_failure_count += 1
                if ck_low_10bp_failure_count >= ck_low_10bp_failure_limit:
                    logger.info(
                        "CK客户Lane构造早停: lane_prefix={}, serial={}, 连续{}次候选10bp低于40%，last={:.1%}, count={}, total={:.1f}G",
                        lane_id_prefix,
                        allocated_lane_serial,
                        ck_low_10bp_failure_count,
                        selected_10bp_ratio,
                        len(selected),
                        selected_total,
                    )
...
def _build_global_round_robin_lane(
    *,
    ...
    ordered_sources = sorted(
        source_groups.items(),
        key=lambda item: (
            min(_get_delivery_priority_sort_value(lib) for lib in item[1]),
...
def _build_lane_validation_cache_key(
    *,
    ...
    # lane_id 主要影响展示和自动编号，不应进入缓存键；否则终态 repair 中每个候选
    # 都会因为 lane_id 不同而失去缓存命中。
    validator_metadata = _build_lane_metadata_for_validator("", lane_metadata, libraries=libraries)
    return (
...
def _pick_10_plus_24_lane_seq_subset(
    ...
    """为10+24 Lane seq选择一组落入容量窗口且Index不冲突的文库。"""
    if not remaining:
        return [], []
    ...
    conflict_masks: List[int] = [0 for _ in remaining]
    for left_index in range(len(remaining)):
        for right_index in range(left_index + 1, len(remaining)):
            if _validate_index_conflicts_latest([remaining[left_index], remaining[right_index]]):
                conflict_masks[left_index] |= 1 << right_index
                conflict_masks[right_index] |= 1 << left_index

    # A single best mask per capacity sum is not enough: two subsets with the
    # same sum can conflict with different downstream libraries. Keep bounded
    # conflict-free states and score them near the target window.
    ...
    for idx, value in enumerate(values):
        ...
        for current_sum, current_mask, current_count in states:
            new_sum = current_sum + value
            if new_sum > max_units:
                continue
            if conflict_masks[idx] & current_mask:
                continue
            new_states.append((new_sum, current_mask | (1 << idx), current_count + 1))
        if len(new_states) > state_limit:
            new_states.sort(
                key=lambda state: (
                    0 if min_units <= state[0] <= max_units else 1,
                    abs(state[0] - target_units),
                    -state[0],
                    -state[2],
                )
            )
            new_states = new_states[:state_limit]
        states = new_states

    candidate_states = [
        state
        for state in states
        if min_units <= state[0] <= max_units
    ]
    if not candidate_states:
        return [], list(remaining)

    candidate_states.sort(key=lambda state: (abs(state[0] - target_units), -state[0], -state[2]))
    selected_indices: Set[int] = set()
    for _, selected_mask, _ in candidate_states:
        candidate_indices = {idx for idx in range(len(remaining)) if selected_mask & (1 << idx)}
        candidate_libraries = [remaining[idx] for idx in sorted(candidate_indices)]
        if _count_lane_index_pairs(candidate_libraries) < AI_LANE_MIN_INDEX_PAIRS:
            continue
        if _validate_index_conflicts_latest(candidate_libraries):
            continue
...
def _try_convert_mergeable_36t_tail_lanes_to_mode_1_1(
    ...
    """将可合并的3.6T Lane转换为1.1 Lane。

    终态合并口径：
    - 包Lane不合并；
    - 拆分文库或按拆分规则禁止进1.1的原始文库不合并；
    - 加测/混合文库可以合并，但合并后单Lane总量不能超过150G；
    - 其他3.6T Lane允许跨来源尝试，最终仍以LaneValidator为准。
    """
    lane_assignments = list(getattr(solution, "lane_assignments", []) or [])
    ...
    committed_returned_libraries: List[EnhancedLibraryInfo] = []
    ...
    def _is_terminal_plain_gm_lane(lane: LaneAssignment) -> bool:
        lane_id = _safe_str(getattr(lane, "lane_id", ""), default="")
        if not lane_id.startswith("GM_"):
            return False
        metadata = getattr(lane, "metadata", {}) or {}
        dispatch_stage = _safe_str(metadata.get("dispatch_stage"), default="")
        return dispatch_stage in {"", "terminal_global_36t_mixed", "terminal_global_36t_split_mixed"}

    def _try_rebuild_gm_group_to_mode_1_1() -> bool:
        """两两不够1.1下限时，从多条GM普通3.6中重选一批1.1库。"""
        nonlocal next_serial
        from itertools import combinations

        gm_indexes = [
            idx for idx, lane in enumerate(ordered_lanes)
            if _is_terminal_plain_gm_lane(lane)
        ]
        if len(gm_indexes) < 2:
            return False
        ...
        max_group_size = min(4, len(gm_indexes))
        for group_size in range(max_group_size, 1, -1):
            for combo in combinations(gm_indexes, group_size):
                if any(idx in selected_source_indexes for idx in combo):
                    continue
                source_lanes = [ordered_lanes[idx] for idx in combo]
                source_libraries: List[EnhancedLibraryInfo] = []
                machine_values: Set[str] = set()
                for source_lane in source_lanes:
                    machine_type = getattr(source_lane, "machine_type", MachineType.NOVA_X_25B)
                    machine_values.add(getattr(machine_type, "value", str(machine_type)))
                    source_libraries.extend(
                        _get_non_balance_libraries(list(getattr(source_lane, "libraries", []) or []))
                    )
                if len(machine_values) != 1 or not source_libraries:
                    continue
                if any(
                    _is_split_library(lib) or _is_split_rule_original_blocked_from_1_1(lib)
                    for lib in source_libraries
                ):
                    continue
                if _mode_1_1_add_test_limited_data_gb(source_libraries) > max_add_test_gb + 1e-6:
                    continue
                ...
                lane_metadata = {
                    ...
                    "seq_mode": "1.1",
                    "lcxms": "1.1",
                    "dispatch_stage": "terminal_36t_group_rebuild_to_mode_1_1",
                    "source_lane_ids": source_lane_ids,
                }
                candidate_results: List[Tuple[float, int, str, LaneAssignment, List[EnhancedLibraryInfo]]] = []
                ...
                def _record_valid_candidate(
                    attempt_name: str,
                    candidate_lane: Optional[LaneAssignment],
                    candidate_used: List[EnhancedLibraryInfo],
                ) -> None:
                    if candidate_lane is None or not candidate_used:
                        return
                    validation_result = _validate_lane_state(
                        validator,
                        candidate_lane,
                        list(getattr(candidate_lane, "libraries", []) or []),
                    )
                    if not getattr(validation_result, "is_valid", False):
                        return
                    candidate_results.append(
                        (
                            _total_lane_data(candidate_used),
                            len(candidate_used),
                            attempt_name,
                            candidate_lane,
                            list(candidate_used),
                ...
                for attempt_name, deterministic_order in (
                    ("full_sorted", None),
                    ("budgeted_ratio", "budgeted_ratio"),
                ):
                    candidate_lane, candidate_used = _attempt_build_lane_from_pool(
                        pool=source_libraries,
                        validator=validator,
                        machine_type=machine_type,
                        lane_id_prefix="GL",
                        lane_serial=next_serial,
                        index_conflict_attempts=40,
                        other_failure_attempts=40,
                        extra_metadata=lane_metadata,
                        prioritize_scattered_mix=False,
                        deterministic_candidate_order=deterministic_order,
                    ...
                    _record_valid_candidate(attempt_name, candidate_lane, candidate_used)

                if not candidate_results:
                    candidate_lane, candidate_used = _try_build_global_mode_1_1_lane_from_pool(
                        pool=source_libraries,
                        validator=validator,
                        machine_type=machine_type,
                        lane_serial=next_serial,
                        lane_id_prefix="GL",
                        exhaustive=True,
                        relaxed_random_failure_attempts=40,
                        near_min_variant_attempts_without_lane=7,
                        near_min_seed_attempts_per_variant=20,
                        near_min_enable_candidate_repair=True,
                        near_min_time_budget_seconds=45.0,
                        fallback_enable_candidate_repair=True,
                        enable_large_pool_trim=False,
                        stop_large_pool_after_weak_near_min=False,
                        lane_metadata_overrides=lane_metadata,
                    )
                    _record_valid_candidate("near_min_fallback", candidate_lane, candidate_used)

                if not candidate_results:
                    continue
                _, _, selected_attempt, lane, used = max(
                    candidate_results,
                ...
                if not isinstance(lane.metadata, dict):
                    lane.metadata = {}
                lane.metadata["dispatch_stage"] = "terminal_36t_group_rebuild_to_mode_1_1"
                lane.metadata["selected_seq_mode"] = "1.1"
                lane.metadata["seq_mode"] = "1.1"
                lane.metadata["lcxms"] = "1.1"
                lane.metadata["source_lane_ids"] = source_lane_ids
                for lib in list(getattr(lane, "libraries", []) or []):
                    lib._current_seq_mode_raw = "1.1"
                    lib.selected_seq_mode = "1.1"
                    lib.current_seq_mode = "1.1"
                ...
                returned_libraries = [
                    lib for lib in source_libraries
                    if id(lib) not in used_ids
                ]
                committed_source_lane_ids.update(id(source_lane) for source_lane in source_lanes)
                committed_new_lanes.append(lane)
                committed_used_library_ids.update(used_ids)
                committed_returned_libraries.extend(returned_libraries)
                selected_source_indexes.update(combo)
                next_serial += 1
                made_conversion = True
                logger.info(
                    "终态GM 3.6组合重选1.1成功: source_lanes={}，attempt={}，新增lane={}，使用{}个/{:.1f}G，回退未用{}个/{:.1f}G",
                    [
                        _safe_str(getattr(source_lane, "lane_id", ""), default="")
                        for source_lane in source_lanes
                    ],
            ...
            if made_conversion:
                break
        return made_conversion

    if not pair_options:
        _try_rebuild_gm_group_to_mode_1_1()

    selected_pair_indexes: List[Tuple[int, int]] = []
    if pair_options and len(ordered_lanes) <= 24:
    ...
    if committed_returned_libraries:
        existing_unassigned = list(getattr(solution, "unassigned_libraries", []) or [])
        existing_unassigned_ids = {id(lib) for lib in existing_unassigned}
        ...
        returned_unique = [
            lib for lib in committed_returned_libraries
            if id(lib) not in existing_unassigned_ids and id(lib) not in assigned_library_ids
        ]
        solution.unassigned_libraries = existing_unassigned + returned_unique
    return {
        ...
        "returned_libraries": len(committed_returned_libraries),
    }
...
def _is_forbidden_in_mode_1_1_by_secondary_36t_policy(lib: EnhancedLibraryInfo) -> bool:
    """识别只允许走3.6T-NEW的文库。"""
    return _is_singapore_process_library(lib)
...
def _validate_lane_special_split_rule(
    ...
    if lane_sample_types & LANE_LOADING_HD_VISIUM_TYPES and lane_sample_types & LANE_LOADING_COMBO_GROUP_B:
        return False, tokens, "hd_visium_and_group_b_mixed"
...
def _resolve_lane_output_rule_fields(
    ...
    sequencing_mode = _output_sequencing_mode_from_selection(selection)
...
def _is_singapore_process_library(lib: EnhancedLibraryInfo) -> bool:
    """识别新加坡工序文库；新加坡只允许走3.6T-NEW，不进入1.1。"""
    for attr_name in ("process_code", "test_code"):
        raw_value = getattr(lib, attr_name, None)
        try:
            parsed = int(float(str(raw_value).strip()))
    ...
    return lab_name in {
        _normalize_text_for_match("新加坡"),
        _normalize_text_for_match("新加坡科技服务实验室"),
    }


def _partition_singapore_libraries_for_36t(
    libraries: Sequence[EnhancedLibraryInfo],
) -> Tuple[List[EnhancedLibraryInfo], List[EnhancedLibraryInfo]]:
    """将新加坡文库从1.1候选池剥离，直接锁定到3.6T-NEW。"""
    singapore_libraries: List[EnhancedLibraryInfo] = []
    remaining_libraries: List[EnhancedLibraryInfo] = []
    for lib in list(libraries or []):
        if _is_singapore_process_library(lib):
            singapore_libraries.append(lib)
        else:
            remaining_libraries.append(lib)
    if singapore_libraries:
        _prepare_libraries_for_3_6t_scheduling(singapore_libraries)
    return singapore_libraries, remaining_libraries
...
def _build_lane_metadata_for_validator(
    ...
    if (
        lane_id.startswith("NB_")
        and not bool((lane_metadata or {}).get("is_pure_10bp_lane"))
        and not bool((lane_metadata or {}).get("is_mixed_10bp_nb_lane"))
    ):
...
def _is_customer_like_validator(lib: EnhancedLibraryInfo) -> bool:
    """按当前业务口径识别客户文库：wksampleid/sample_id 第2-4位为 KDL。"""
    sample_id = _safe_library_text(lib, "sample_id", "wksampleid")
    return sample_id.upper()[1:4] == "KDL"
...
def _try_add_matrix_split_lanes_from_unassigned(
    ...
    for split_count, libs in sorted(grouped.items(), key=lambda item: (-item[0], -len(item[1]))):
        ...
        remaining = _sort_remaining_for_scattered_mix_lane(remaining)
...
def _find_cross_split_source_subset(
    ...
    ordered = sorted(
        ...
        key=lambda item: (
            _get_delivery_priority_sort_value(item[0]),
...
def _try_build_single_36t_mixed_lane_from_items(
    *,
    ...
    """从普通文库中挑一条3.6T混排Lane，复用1.1近下限的多变体构造思路。

    拆分文库必须以完整家族跨多条Lane提交，且同源片段不能进入同一条Lane；
    单条增量Lane无法满足该原子性要求，因此这里只处理普通补料。
    """
    source_by_identity: Dict[str, int] = {}
    candidates: List[EnhancedLibraryInfo] = []
    for filler in list(filler_libraries or []):
        if _is_ai_balance_library(filler) or _is_split_library(filler):
            continue
        if not _is_normal_filler_allowed_for_36t(filler):
            continue
        identity = _get_library_identity_key(filler)
        if identity in source_by_identity:
            continue
        source_by_identity[identity] = id(filler)
    ...
    best_total = 0.0
    best_index_pairs = 0
    best_repair_libs: List[EnhancedLibraryInfo] = []
    ...
    active_candidates = _filter_libraries_by_hard_priority(
        sorted(
            candidates,
            key=lambda lib: (
                _get_delivery_priority_sort_value(lib),
                0 if _is_imbalance_library_candidate(lib) else 1,
                -_count_library_index_pairs(lib),
                -float(getattr(lib, "contract_data_raw", 0.0) or lib.get_data_amount_gb() or 0.0),
                _get_library_identity_key(lib),
            ),
        ...
        machine_type=machine_type,
        lane_metadata=lane_metadata,
        stage_name=f"{lane_id_prefix}_terminal_global_36t_mixed",
        emit_log=False,
    )
    if not active_candidates:
        return None, set()

    directed_lane, directed_selected = _try_build_single_36t_pure_customer_lane(
        active_candidates,
        validator=validator,
        lane_id_prefix="CK36",
        machine_type=machine_type,
    )
    if directed_lane is not None and directed_selected:
        used_sources = {
            source_by_identity.get(_get_library_identity_key(item), id(item))
            for item in directed_selected
        }
        return directed_lane, used_sources

    special_data_limit = _resolve_special_library_data_limit(machine_type)
    idx_validator = _MODULE_IDX_VALIDATOR
    max_validation_attempts = 90
    validation_attempts = 0
    repair_impossible_counters: Counter[str] = Counter()
    terminal_impossible_reason = ""

    def _repair_impossible_reason(
        items: List[EnhancedLibraryInfo],
        min_allowed: float,
        max_allowed: float,
    ) -> str:
        real_items = [item for item in items if not _is_ai_balance_library(item)]
        if not real_items:
            return ""

        def _item_data(item: EnhancedLibraryInfo) -> float:
            return float(getattr(item, "contract_data_raw", 0.0) or item.get_data_amount_gb() or 0.0)

        total_data = sum(_item_data(item) for item in real_items)
        if total_data <= 0:
            return ""

        customer_predicate = getattr(validator, "_is_customer_side_library", None)
        if callable(customer_predicate):
            customer_items = [item for item in real_items if customer_predicate(item)]
            customer_data = sum(_item_data(item) for item in customer_items)
            non_customer_data = total_data - customer_data
            customer_ratio = customer_data / total_data if total_data > 0 else 0.0
            if customer_ratio > 0.50 + 1e-9 and customer_ratio < 1.0 - 1e-9:
                # 修复客户占比只有两条路：
                # 1) 加/换足够非客户文库，把客户占比稀释到 <=50%，要求总量至少 2*customer_data；
                # 2) 剔除所有非客户文库，变成 100% 客户 Lane，要求客户文库自身仍满足容量下限。
                dilute_possible_by_capacity = (2.0 * customer_data) <= max_allowed + 1e-6
                selected_customer_ids = {id(item) for item in customer_items}
                available_customer_data = customer_data + sum(
                    _item_data(item)
                    for item in active_candidates
                    if id(item) not in selected_customer_ids
                    and customer_predicate(item)
                )
                # 转纯客户Lane时可以先剔除非客户，再从同一3.6候选池补客户文库；
                # 因此不能只看当前候选内客户数据，否则会误判 324 这类客户池充足的场景。
                pure_customer_possible_by_capacity = (
                    available_customer_data >= min_allowed - 1e-6
                    and _count_lane_index_pairs(customer_items) >= AI_LANE_MIN_INDEX_PAIRS
                )
                if not dilute_possible_by_capacity and not pure_customer_possible_by_capacity:
                    return (
                        "customer_ratio_unrepairable_by_capacity("
                        f"customer={customer_data:.1f}G,non_customer={non_customer_data:.1f}G,"
                        f"ratio={customer_ratio:.1%},available_customer={available_customer_data:.1f}G,"
                        f"min={min_allowed:.1f}G,max={max_allowed:.1f}G)"
                    )

        libs_10bp, libs_non_10bp = _split_10bp_and_non_10bp(real_items, validator)
        data_10bp = sum(_item_data(item) for item in libs_10bp)
        data_non_10bp = sum(_item_data(item) for item in libs_non_10bp)
        mixed_10bp = data_10bp > 0 and data_non_10bp > 0
        if mixed_10bp:
            ratio_10bp = data_10bp / (data_10bp + data_non_10bp)
            if ratio_10bp < 0.40 - 1e-9:
                required_add_10bp = max(0.0, (0.40 * total_data - data_10bp) / 0.60)
                add_possible_by_capacity = total_data + required_add_10bp <= max_allowed + 1e-6
                max_non_10bp_after_trim = 1.5 * data_10bp
                total_after_trim = data_10bp + min(data_non_10bp, max_non_10bp_after_trim)
                trim_possible_by_capacity = total_after_trim >= min_allowed - 1e-6
                selected_10bp_ids = {id(item) for item in libs_10bp}
                available_10bp_data = data_10bp + sum(
                    _item_data(item)
                    for item in active_candidates
                    if id(item) not in selected_10bp_ids
                    and item in _split_10bp_and_non_10bp([item], validator)[0]
                )
                pool_rebuild_possible = available_10bp_data >= (0.40 * min_allowed) - 1e-6
                if not add_possible_by_capacity and not trim_possible_by_capacity and not pool_rebuild_possible:
                    return (
                        "10bp_ratio_unrepairable_by_capacity("
                        f"10bp={data_10bp:.1f}G,non10bp={data_non_10bp:.1f}G,"
                        f"ratio={ratio_10bp:.1%},need_add={required_add_10bp:.1f}G,"
                        f"available_10bp={available_10bp_data:.1f}G,"
                        f"min={min_allowed:.1f}G,max={max_allowed:.1f}G)"
                    )
        return ""
    ...
    def build_lane(
        lane_id: str,
        lane_libs: List[EnhancedLibraryInfo],
    ) -> LaneAssignment:
        lane = LaneAssignment(
            lane_id=lane_id,
            machine_id=f"M_{lane_id}",
    ...
    def record_best(selected: List[EnhancedLibraryInfo]) -> None:
        nonlocal best_total, best_index_pairs
        total = _total_lane_data(selected)
        if total > best_total + 1e-6:
            best_total = total
    ...
    def validate_completed(selected: List[EnhancedLibraryInfo]) -> Optional[LaneAssignment]:
        nonlocal validation_attempts
        nonlocal best_repair_libs, best_repair_total, best_repair_index_pairs
        nonlocal terminal_impossible_reason
        if terminal_impossible_reason:
            return None
        if validation_attempts >= max_validation_attempts:
            return None
        total = _total_lane_data(selected)
        ...
        if _count_lane_index_pairs(selected) < AI_LANE_MIN_INDEX_PAIRS:
            validation_failures["index_pairs_below_min"] += 1
            return None
        if require_36t_main_library and not any(_is_eligible_for_36t_after_mode_priority(item) for item in selected):
            validation_failures["missing_36t_main_library"] += 1
            return None
        trial_index_pairs = _count_lane_index_pairs(selected)
        if total > best_repair_total + 1e-6 or (
            abs(total - best_repair_total) <= 1e-6
            and trial_index_pairs > best_repair_index_pairs
        ):
            best_repair_libs = list(selected)
            best_repair_total = total
            best_repair_index_pairs = trial_index_pairs
        validation_attempts += 1
        trial_lane = build_lane(f"{lane_id_prefix}_TMP", selected)
        if _is_split_lane_forbidden_by_mode(trial_lane):
            validation_failures["split_forbidden_by_mode"] += 1
            return None
        validation_result = _validate_lane_state(validator, trial_lane, selected)
        if not getattr(validation_result, "is_valid", False):
            for err in list(getattr(validation_result, "errors", []) or [])[:4]:
                key = _safe_str(getattr(err, "message", None), default="validation_error")
                validation_failures[key] += 1
            if getattr(validator, "strict_mode", False):
                for warn in list(getattr(validation_result, "warnings", []) or [])[:2]:
                    key = _safe_str(getattr(warn, "message", None), default="validation_warning")
                    validation_failures[key] += 1
            repair_skip_reason = _repair_impossible_reason(selected, min_allowed, max_allowed)
            if repair_skip_reason:
                repair_skip_root = repair_skip_reason.split("(", 1)[0]
                repair_impossible_counters[repair_skip_root] += 1
                if repair_impossible_counters[repair_skip_root] >= 12:
                    terminal_impossible_reason = repair_skip_root
                validation_failures[repair_skip_reason] += 1
                return None
            repaired_lane, repaired_selected, repair_action = _build_repaired_candidate_lane(
                libraries=list(selected),
                pool=active_candidates,
                validator=validator,
                machine_type=machine_type,
                lane_id=f"{lane_id_prefix}_{machine_type.value}_{_reserve_auto_lane_serial(lane_id_prefix, machine_type):03d}",
                lane_metadata=lane_metadata,
                stage_label=f"{lane_id_prefix}_terminal_global_36t_near_min_repair",
                max_fill_candidates=60,
                max_replace_remove_candidates=12,
                max_replace_add_candidates=40,
                deadline=deadline,
            )
            if repaired_lane is not None:
                if repair_action != "already_valid":
                    logger.info(
                        "终态全局3.6近下限通用修复成功: lane={}, action={}, 原始{}个/{:.1f}G, 修复后{}个/{:.1f}G",
                        repaired_lane.lane_id,
                        repair_action,
                        len(selected),
                        _total_lane_data(selected),
            ...
            return None
        lane_id = f"{lane_id_prefix}_{machine_type.value}_{_reserve_auto_lane_serial(lane_id_prefix, machine_type):03d}"
        return build_lane(lane_id, selected)

    def candidate_sort_key(lib: EnhancedLibraryInfo, variant: str) -> Tuple[Any, ...]:
        data = float(getattr(lib, "contract_data_raw", 0.0) or lib.get_data_amount_gb() or 0.0)
        if variant == "small_first":
            data_key = data
        elif variant == "balanced_fill":
    ...
    def _is_10bp_for_36t_probe(item: EnhancedLibraryInfo) -> bool:
        ten_bp_data = getattr(item, "ten_bp_data", None)
        if ten_bp_data is not None and ten_bp_data > 0:
            return True
        return bool(getattr(validator, "_is_10bp_index", lambda _: False)(getattr(item, "index_seq", "") or ""))

    def _tenbp_shape_valid_for_36t_probe(tenbp_total: float, non10_total: float) -> bool:
        if tenbp_total <= 0.0 or non10_total <= 0.0:
            return True
        return tenbp_total / (tenbp_total + non10_total) >= 0.40 - 1e-6

    def _try_36t_feasibility_probe(
        candidates_for_probe: List[EnhancedLibraryInfo],
        *,
        state_limit: int = 384,
    ) -> Optional[LaneAssignment]:
        """有界DP/beam可行性探针：覆盖普通贪心补不上的3.6候选组合。"""
        if not candidates_for_probe:
            return None
        min_allowed, max_allowed = _resolve_lane_capacity_limits(
            libraries=candidates_for_probe,
            machine_type=machine_type,
            lane_id=f"{lane_id_prefix}_TMP",
            lane_metadata=lane_metadata,
        )
        absolute_imbalance_limit = min(
            special_data_limit,
            max_allowed * CUSTOMER_MIX_IMBALANCE_MAX_RATIO,
        )

        def _state_score(state: Dict[str, Any]) -> Tuple[Any, ...]:
            total = float(state["total"])
            tenbp_total = float(state["tenbp"])
            non10_total = float(state["non10"])
            imbalance_total = float(state["imbalance"])
            in_capacity = min_allowed <= total <= max_allowed
            ratio_ok = (
                total > 0.0
                and imbalance_total / total <= CUSTOMER_MIX_IMBALANCE_MAX_RATIO + 1e-6
                and _tenbp_shape_valid_for_36t_probe(tenbp_total, non10_total)
            )
            imbalance_ratio = imbalance_total / total if total > 0.0 else 0.0
            return (
                0 if in_capacity and ratio_ok else 1,
                abs(total - min_allowed) if total >= min_allowed else min_allowed - total,
                _imbalance_target_distance(imbalance_ratio, CUSTOMER_MIX_IMBALANCE_MAX_RATIO),
                -total,
                -int(state["index_pairs"]),
                len(state["items"]),
            )
        ...
        best_probe_total = 0.0
        best_probe_index_pairs = 0
        best_probe_items: List[EnhancedLibraryInfo] = []
        best_capacity_shape_total = 0.0
        best_capacity_shape_index_pairs = 0
        validation_candidate_count = 0
        probe_rejects: Counter[str] = Counter()

        ordered = sorted(
            candidates_for_probe,
            key=lambda item: (
                0 if _is_imbalance_library_candidate(item) else 1,
                0 if not _is_10bp_for_36t_probe(item) else 1,
                _get_delivery_priority_sort_value(item),
                -float(getattr(item, "contract_data_raw", 0.0) or item.get_data_amount_gb() or 0.0),
                -_count_library_index_pairs(item),
                _get_library_identity_key(item),
            ),
        )
        for candidate in ordered:
            if deadline is not None and time.monotonic() >= deadline:
                probe_rejects["time_budget_exhausted"] += 1
                break
            candidate_data = float(getattr(candidate, "contract_data_raw", 0.0) or candidate.get_data_amount_gb() or 0.0)
            if candidate_data <= 0.0:
                continue
            candidate_is_imbalance = _is_imbalance_library_candidate(candidate)
            candidate_is_10bp = _is_10bp_for_36t_probe(candidate)
            additions: List[Dict[str, Any]] = []
            for state in states:
                items = list(state["items"])
                if _shares_split_family_with_selected(items, candidate):
                    probe_rejects["split_family"] += 1
                    continue
                total = float(state["total"]) + candidate_data
                if total > max_allowed + 1e-6:
                    probe_rejects["over_max"] += 1
                    continue
                imbalance_total = float(state["imbalance"]) + (candidate_data if candidate_is_imbalance else 0.0)
                if imbalance_total > absolute_imbalance_limit + 1e-6:
                    probe_rejects["imbalance_absolute"] += 1
                    continue
                if total >= min_allowed - 1e-6 and imbalance_total / total > CUSTOMER_MIX_IMBALANCE_MAX_RATIO + 1e-6:
                    probe_rejects["imbalance_ratio"] += 1
                    continue
                trial_items = items + [candidate]
                ss_valid, _, _ = _validate_lane_special_split_rule(trial_items)
                if not ss_valid:
                    probe_rejects["special_split"] += 1
                    continue
                has_conflict, candidate_indices = _new_lib_has_latest_index_conflict_with_cache(
                    state["indices"],
                    candidate,
                )
                if has_conflict:
                    probe_rejects["incremental_index"] += 1
                    continue
                tenbp_total = float(state["tenbp"]) + (candidate_data if candidate_is_10bp else 0.0)
                non10_total = float(state["non10"]) + (0.0 if candidate_is_10bp else candidate_data)
                if total >= min_allowed - 1e-6 and not _tenbp_shape_valid_for_36t_probe(tenbp_total, non10_total):
                    probe_rejects["tenbp_shape"] += 1
                    continue
                additions.append(
                    {
                        "items": trial_items,
                        "indices": list(state["indices"]) + [candidate_indices],
                        "total": total,
                        "imbalance": imbalance_total,
                        "tenbp": tenbp_total,
                        "non10": non10_total,
                        "index_pairs": int(state["index_pairs"]) + _count_library_index_pairs(candidate),
                    }
                )
            if additions:
                states.extend(additions)
                states.sort(key=_state_score)
                states = states[:state_limit]
                for state in states[: min(48, len(states))]:
                    total = float(state["total"])
                    if total > best_probe_total + 1e-6:
                        best_probe_total = total
                        best_probe_index_pairs = int(state["index_pairs"])
                        best_probe_items = list(state["items"])
                    if total < min_allowed - 1e-6 or total > max_allowed + 1e-6:
                        continue
                    if int(state["index_pairs"]) < AI_LANE_MIN_INDEX_PAIRS:
                        continue
                    if float(state["imbalance"]) / total > CUSTOMER_MIX_IMBALANCE_MAX_RATIO + 1e-6:
                        continue
                    if not _tenbp_shape_valid_for_36t_probe(float(state["tenbp"]), float(state["non10"])):
                        continue
                    validation_candidate_count += 1
                    if total > best_capacity_shape_total + 1e-6:
                        best_capacity_shape_total = total
                        best_capacity_shape_index_pairs = int(state["index_pairs"])
                    lane = validate_completed(list(state["items"]))
                    if lane is not None:
                        return lane
        repair_seed_threshold = max(0.0, 0.90 * min_allowed)
        if best_probe_items and best_probe_total >= repair_seed_threshold - 1e-6:
            best_selected_ids = {id(item) for item in best_probe_items}
            best_indices = [_parse_library_index_pairs_latest(item) for item in best_probe_items]
            best_tenbp_total = sum(
                float(getattr(item, "contract_data_raw", 0.0) or item.get_data_amount_gb() or 0.0)
                for item in best_probe_items
                if _is_10bp_for_36t_probe(item)
            )
            best_non10_total = best_probe_total - best_tenbp_total
            best_imbalance_total = sum(
                float(getattr(item, "contract_data_raw", 0.0) or item.get_data_amount_gb() or 0.0)
                for item in best_probe_items
                if _is_imbalance_library_candidate(item)
            )
            gap_fill_candidates = sorted(
                [
                    item for item in candidates_for_probe
                    if id(item) not in best_selected_ids
                    and not _shares_split_family_with_selected(best_probe_items, item)
                ],
                key=lambda item: (
                    abs(float(getattr(item, "contract_data_raw", 0.0) or item.get_data_amount_gb() or 0.0) - (min_allowed - best_probe_total)),
                    0 if _is_10bp_for_36t_probe(item) else 1,
                    0 if _is_imbalance_library_candidate(item) else 1,
                    _get_delivery_priority_sort_value(item),
                    -_count_library_index_pairs(item),
                    _get_library_identity_key(item),
            ...
            gap_rejects: Counter[str] = Counter()
            direct_gap_trials: List[List[EnhancedLibraryInfo]] = []
            for first in gap_fill_candidates[:80]:
                first_data = float(getattr(first, "contract_data_raw", 0.0) or first.get_data_amount_gb() or 0.0)
                first_total = best_probe_total + first_data
                if first_total <= max_allowed + 1e-6:
                    direct_gap_trials.append([first])
                for second in gap_fill_candidates[:40]:
                    if second is first or id(second) in best_selected_ids:
                        continue
                    if _shares_split_family_with_selected([*best_probe_items, first], second):
                        continue
                    second_data = float(getattr(second, "contract_data_raw", 0.0) or second.get_data_amount_gb() or 0.0)
                    pair_total = best_probe_total + first_data + second_data
                    if pair_total <= max_allowed + 1e-6:
                        direct_gap_trials.append([first, second])
            direct_gap_trials.sort(
                key=lambda additions: (
                    abs((best_probe_total + sum(
                        float(getattr(item, "contract_data_raw", 0.0) or item.get_data_amount_gb() or 0.0)
            ...
            for additions in direct_gap_trials[:160]:
                trial = list(best_probe_items) + list(additions)
                trial_total = _total_lane_data(trial)
                if trial_total < min_allowed - 1e-6 or trial_total > max_allowed + 1e-6:
                    gap_rejects["capacity"] += 1
                    continue
                trial_imbalance = sum(
                    float(getattr(item, "contract_data_raw", 0.0) or item.get_data_amount_gb() or 0.0)
                    for item in trial
                ...
                if trial_imbalance / trial_total > CUSTOMER_MIX_IMBALANCE_MAX_RATIO + 1e-6:
                    gap_rejects["imbalance_ratio"] += 1
                    continue
                trial_tenbp = sum(
                    float(getattr(item, "contract_data_raw", 0.0) or item.get_data_amount_gb() or 0.0)
                    for item in trial
                ...
                trial_non10 = trial_total - trial_tenbp
                if not _tenbp_shape_valid_for_36t_probe(trial_tenbp, trial_non10):
                    gap_rejects["tenbp_shape"] += 1
                    continue
                conflict = False
                trial_indices = list(best_indices)
                for addition in additions:
                    has_conflict, addition_indices = _new_lib_has_latest_index_conflict_with_cache(
                        trial_indices,
                        addition,
                    )
                    if has_conflict:
                        conflict = True
                        break
                    trial_indices.append(addition_indices)
                if conflict:
                    gap_rejects["incremental_index"] += 1
                    continue
                lane = validate_completed(trial)
                if lane is not None:
                    return lane
            repair_lane_id = f"{lane_id_prefix}_{machine_type.value}_{_reserve_auto_lane_serial(lane_id_prefix, machine_type):03d}"
            repaired_lane, repaired_selected, repair_action = _build_repaired_candidate_lane(
                libraries=list(best_probe_items),
                pool=active_candidates,
                validator=validator,
                machine_type=machine_type,
                lane_id=repair_lane_id,
                lane_metadata=lane_metadata,
                stage_label=f"{lane_id_prefix}_terminal_global_36t_dp_probe_repair",
    ...
    probe_lane = _try_36t_feasibility_probe(active_candidates)
    if probe_lane is not None:
        used_sources = {
            source_by_identity.get(_get_library_identity_key(item), id(item))
            for item in list(getattr(probe_lane, "libraries", []) or [])
            if not _is_ai_balance_library(item)
        }
        return probe_lane, used_sources

    variant_plan = ("large_first", "small_first", "balanced_fill")
    for variant in variant_plan:
        if terminal_impossible_reason:
            break
        ordered = sorted(active_candidates, key=lambda lib: candidate_sort_key(lib, variant))
        seed_candidates = [
            lib for lib in ordered
            if (not require_36t_main_library or _is_eligible_for_36t_after_mode_priority(lib))
        ...
        for seed_index, seed in enumerate(seed_candidates[:40], start=1):
            if terminal_impossible_reason:
                break
            if validation_attempts >= max_validation_attempts:
                break
            ...
            selected_idx_cache: List[Any] = []
            total = 0.0
            for lib in [seed] + [item for item in ordered if id(item) != id(seed)]:
                if terminal_impossible_reason:
                    break
                if validation_attempts >= max_validation_attempts:
                    break
                ...
                if total + data > max_allowed + 1e-6:
                    rejected_counters["over_max"] += 1
                    continue
                projected_imbalance_data = _project_lane_special_data(selected, lib)
                projected_total = total + data
                if projected_imbalance_data > special_data_limit + SPECIAL_LIBRARY_LIMIT_EPSILON:
                    rejected_counters["special_limit"] += 1
                    continue
                if projected_total > 0 and projected_imbalance_data / projected_total > CUSTOMER_MIX_IMBALANCE_MAX_RATIO + 1e-6:
                    rejected_counters["imbalance_ratio"] += 1
                    continue
                ss_valid, _, _ = _validate_lane_special_split_rule(trial_libs)
                if not ss_valid:
                ...
                selected_idx_cache.append(lib_indices)
                total += data
                record_best(selected)
                lane = validate_completed(selected)
                if lane is not None:
                    used_sources = {
                        source_by_identity.get(_get_library_identity_key(item), id(item))
                        for item in list(getattr(lane, "libraries", []) or [])
                        if not _is_ai_balance_library(item)
                    }
                    logger.info(
                        "终态全局3.6近下限混排成功: lane={}, variant={}, 使用{}个/{:.1f}G",
                        lane.lane_id,
                        variant,
                        len(list(getattr(lane, "libraries", []) or [])),
                        _total_lane_data(list(getattr(lane, "libraries", []) or [])),
                    )
                    return lane, used_sources

    if best_repair_libs and not terminal_impossible_reason:
        repair_min_allowed, repair_max_allowed = _resolve_lane_capacity_limits(
            libraries=best_repair_libs,
            machine_type=machine_type,
            lane_id=f"{lane_id_prefix}_TMP",
            lane_metadata=lane_metadata,
        )
        final_repair_skip_reason = _repair_impossible_reason(
            best_repair_libs,
            repair_min_allowed,
            repair_max_allowed,
        )
        if not final_repair_skip_reason:
            repair_lane_id = f"{lane_id_prefix}_{machine_type.value}_{_reserve_auto_lane_serial(lane_id_prefix, machine_type):03d}"
            repair_seed_lane = build_lane(repair_lane_id, best_repair_libs)
            repair_seed_libs = list(getattr(repair_seed_lane, "libraries", []) or [])
            if (not require_36t_main_library) or any(_is_eligible_for_36t_after_mode_priority(item) for item in repair_seed_libs):
                repaired_lane, repaired_selected, repair_action = _build_repaired_candidate_lane(
                ...
                if repaired_lane is not None:
                    ...
                    repaired_used_sources = {
                        source_by_identity.get(_get_library_identity_key(lib), id(lib))
                        for lib in repaired_selected
                        if _get_library_identity_key(lib) in repaired_identity_keys
                    }
                    return repaired_lane, repaired_used_sources
    logger.info(
        "终态全局3.6增量混排未成Lane明细: 机型={}, 候选items={}, best_total={:.1f}G, best_index_pairs={}, repair_seed={:.1f}G/{}对index, rejects={}, validation_top={}".format(
            machine_type.value,
            ...
            dict(rejected_counters.most_common(8)),
            validation_failures.most_common(5),
        )
    )
    return None, set()
...
def _try_build_single_36t_pure_customer_lane(
    libraries: List[EnhancedLibraryInfo],
    *,
    validator: Any,
    lane_id_prefix: str = "CK36",
    machine_type: MachineType = MachineType.NOVA_X_25B,
) -> Tuple[Optional[LaneAssignment], List[EnhancedLibraryInfo]]:
    """3.6T入口优先构造100% KDL客户纯Lane。"""

    def _data(item: EnhancedLibraryInfo) -> float:
        return float(getattr(item, "contract_data_raw", 0.0) or item.get_data_amount_gb() or 0.0)

    def _total(items: Sequence[EnhancedLibraryInfo]) -> float:
        return sum(_data(item) for item in items)

    def _peak(item: EnhancedLibraryInfo) -> float:
        return float(getattr(item, "peak_size", 0.0) or 0.0)

    def _is_10bp_item(item: EnhancedLibraryInfo) -> bool:
        ten_bp_data = getattr(item, "ten_bp_data", None)
        if ten_bp_data is not None and ten_bp_data > 0:
            return True
        return bool(getattr(validator, "_is_10bp_index", lambda _: False)(getattr(item, "index_seq", "") or ""))
    ...
    metadata = {
        ...
        "seq_mode": "3.6T-NEW",
        "lcxms": "3.6T-NEW",
        "dispatch_stage": "preextract_36t_pure_customer",
        "is_pure_non_10bp_lane": True,
    }
    candidate_pool = [
        lib for lib in list(libraries or [])
        if _is_customer_library_candidate(lib)
        and not _is_ai_balance_library(lib)
        and not _is_split_library(lib)
        and not _is_10bp_item(lib)
        and _peak(lib) > 0
        and _is_normal_filler_allowed_for_36t(lib)
    ]
    if not candidate_pool:
        return None, []

    min_allowed, max_allowed = _resolve_lane_capacity_limits(
        libraries=candidate_pool,
        machine_type=machine_type,
        lane_id=f"{lane_id_prefix}_TMP",
        lane_metadata=metadata,
    )
    window_bp = int(getattr(validator, "peak_size_max_diff", 150) or 150)
    coverage_ratio = float(getattr(validator, "peak_size_coverage_ratio", 0.75) or 0.75)
    imbalance_limit = float(
        getattr(validator, "base_imbalance_ratio_limit_3_6t", CUSTOMER_MIX_IMBALANCE_MAX_RATIO)
        or CUSTOMER_MIX_IMBALANCE_MAX_RATIO
    )

    def _imbalance_ratio(items: Sequence[EnhancedLibraryInfo]) -> float:
        total = _total(items)
        if total <= 0:
            return 0.0
        return sum(_data(item) for item in items if _is_imbalance_library_candidate(item)) / total

    def _make_lane(items: List[EnhancedLibraryInfo], lane_id: str) -> LaneAssignment:
        lane = LaneAssignment(
            lane_id=lane_id,
            machine_id=f"M_{lane_id}",
            machine_type=machine_type,
    ...
    def _validate_selection(items: List[EnhancedLibraryInfo]) -> bool:
        total = _total(items)
        if total < min_allowed - 1e-6 or total > max_allowed + 1e-6:
            return False
        if _count_lane_index_pairs(items) < AI_LANE_MIN_INDEX_PAIRS:
            return False
        if _imbalance_ratio(items) > imbalance_limit + 1e-6:
            return False
        trial_lane = _make_lane(items, f"{lane_id_prefix}_TMP")
        result = _validate_lane_state(validator, trial_lane, items)
        return bool(getattr(result, "is_valid", False))

    def _try_add(
        selected: List[EnhancedLibraryInfo],
        selected_indices: List[Tuple[Tuple[str, Optional[str]], ...]],
        candidate: EnhancedLibraryInfo,
    ) -> Optional[Tuple[Tuple[str, Optional[str]], ...]]:
        if _total(selected) + _data(candidate) > max_allowed + 1e-6:
            return None
        if _imbalance_ratio([*selected, candidate]) > imbalance_limit + 1e-6:
            return None
        has_conflict, candidate_indices = _new_lib_has_latest_index_conflict_with_cache(
            selected_indices,
            candidate,
    ...
    for window_start in sorted({int(_peak(item)) for item in candidate_pool}):
        window_end = window_start + window_bp
        inside = [item for item in candidate_pool if window_start <= _peak(item) <= window_end]
        if _total(inside) < min_allowed * coverage_ratio - 1e-6:
            continue
        outside = [item for item in candidate_pool if not (window_start <= _peak(item) <= window_end)]
        sort_key = lambda item: (
            1 if _is_imbalance_library_candidate(item) else 0,
            _get_delivery_priority_sort_value(item),
            -_data(item),
            _safe_str(getattr(item, "origrec", ""), default=""),
        )
        ...
        selected_indices: List[Tuple[Tuple[str, Optional[str]], ...]] = []
        inside_data = 0.0
        for candidate in sorted(inside, key=sort_key):
            candidate_indices = _try_add(selected, selected_indices, candidate)
            if candidate_indices is None:
                continue
            selected.append(candidate)
            selected_indices.append(candidate_indices)
            inside_data += _data(candidate)
            if _validate_selection(selected):
                lane_id = f"{lane_id_prefix}_{machine_type.value}_{_reserve_auto_lane_serial(lane_id_prefix, machine_type):03d}"
                return _make_lane(selected, lane_id), selected

        for candidate in sorted(outside, key=sort_key):
            projected_total = _total(selected) + _data(candidate)
            if projected_total <= 0 or inside_data / projected_total < coverage_ratio - 1e-6:
                continue
            candidate_indices = _try_add(selected, selected_indices, candidate)
            if candidate_indices is None:
                continue
...
def _preextract_36t_pure_customer_lanes(
    libraries: List[EnhancedLibraryInfo],
    *,
    validator: Any,
    max_lanes: int = 8,
) -> Tuple[List[LaneAssignment], List[EnhancedLibraryInfo], Dict[str, int]]:
    """3.6T入口优先抽取100% KDL客户纯Lane。"""
    remaining = list(libraries or [])
    lanes: List[LaneAssignment] = []
    stats = {"pure_customer_lanes": 0, "used_libraries": 0}
    for _ in range(max_lanes):
        lane, selected = _try_build_single_36t_pure_customer_lane(
            remaining,
            validator=validator,
            lane_id_prefix="CK36",
            machine_type=MachineType.NOVA_X_25B,
        )
        if lane is None or not selected:
            break
        lanes.append(lane)
        selected_keys = {_get_library_identity_key(item) for item in selected}
        remaining = [
            item for item in remaining
            if _get_library_identity_key(item) not in selected_keys
        ]
        stats["pure_customer_lanes"] += 1
        stats["used_libraries"] += len(selected)
    return lanes, remaining, stats
...
def _repair_g55_selection_special_splits(
    ...
    *,
    ...
    if ss_valid or ss_reason != "group_a_and_group_b_mixed":
        return None
    removals = _pick_special_split_removals(selected, prefer_delivery_priority=True)
    if not removals:
        return None
    removed_modes = {
...
def _fill_g55_repaired_selection(
    working: List[EnhancedLibraryInfo],
    *,
    repair_pool: List[EnhancedLibraryInfo],
    min_allowed: float,
    ...
    for lib in fill_candidates[:160]:
        if _g55_real_data_gb(working) >= min_allowed - 1e-6:
            break
        lib_data = float(getattr(lib, "contract_data_raw", 0.0) or 0.0)
        if _g55_real_data_gb(working) + lib_data > max_allowed + 1e-6:
...
def _repair_g55_selection_mix_limits(
    selected: List[EnhancedLibraryInfo],
    *,
    repair_pool: List[EnhancedLibraryInfo],
    min_allowed: float,
    ...
    """G55专Lane修复可剔除的1.1限制：G58/G54互斥、加测/混合150G封顶。"""
    if not selected:
        return None

    def _cap_trim_variants(seed: List[EnhancedLibraryInfo]) -> List[List[EnhancedLibraryInfo]]:
        if _mode_1_1_add_test_limited_data_gb(seed) <= float(max_add_test_gb_per_lane or 0.0) + 1e-6:
            return [list(seed)]
        limited = [lib for lib in seed if _is_mode_1_1_add_test_limited_library(lib)]
        ...
        for removal_order in removal_orders:
            working = list(seed)
            for lib in removal_order:
                if _mode_1_1_add_test_limited_data_gb(working) <= float(max_add_test_gb_per_lane or 0.0) + 1e-6:
                    break
                working = [item for item in working if item is not lib]
            if working:
                variants.append(working)
        return variants

    def _ratio_trim_variants(seed: List[EnhancedLibraryInfo], predicate, limit: float) -> List[List[EnhancedLibraryInfo]]:
        total = _g55_real_data_gb(seed)
        if total <= 0:
            return []
        ratio_gb = sum(float(getattr(lib, "contract_data_raw", 0.0) or 0.0) for lib in seed if predicate(lib))
        if ratio_gb / total <= limit + 1e-6:
            return [list(seed)]
        limited = [lib for lib in seed if predicate(lib)]
        variants: List[List[EnhancedLibraryInfo]] = []
        removal_orders = [
            sorted(
                limited,
        ...
        for removal_order in removal_orders:
            ...
            for lib in removal_order:
                ...
                if total <= 0:
                    break
                ratio_gb = sum(float(getattr(item, "contract_data_raw", 0.0) or 0.0) for item in working if predicate(item))
                if ratio_gb / total <= limit + 1e-6:
                    break
                working = [item for item in working if item is not lib]
            if working:
    ...
    buckets = {_g55_type_bucket(lib) for lib in selected}
    base_variants: List[List[EnhancedLibraryInfo]] = []
    if "G58" in buckets and "G54" in buckets:
        for bucket_to_remove in ("G54", "G58"):
            variant = [lib for lib in selected if _g55_type_bucket(lib) != bucket_to_remove]
            if variant:
                base_variants.append(variant)
    else:
        base_variants.append(list(selected))

    seen: Set[Tuple[str, ...]] = set()
    final_variants: List[List[EnhancedLibraryInfo]] = []
    for variant in base_variants:
        for cap_variant in _cap_trim_variants(variant):
            for high_variant in _ratio_trim_variants(cap_variant, _is_g55_high_phix_library, 0.30):
                for customer_variant in _ratio_trim_variants(high_variant, _is_customer_library_candidate, 0.50):
                    signature = tuple(sorted(_get_library_identity_key(lib) for lib in customer_variant))
                    if signature in seen:
                        continue
                    seen.add(signature)
                    final_variants.append(customer_variant)

    final_variants.sort(
        key=lambda variant: (
            abs(_g55_real_data_gb(variant) - soft_target),
            -_g55_real_data_gb(variant),
            -_count_lane_index_pairs(variant),
        )
    )
    for variant in final_variants:
        repaired = _fill_g55_repaired_selection(
            list(variant),
...
def _is_g55_mode_1_1_candidate(lib: EnhancedLibraryInfo) -> bool:
    ...
    if not _resolve_g55_base_group_id_1_1_legacy(lib):
        return False
    if _is_split_library(lib):
        return False
...
def _consume_g55_imbalance_as_mode_lanes(
    *,
    ...
    while len(lanes) < max_lanes:
        ...
        best_used: List[EnhancedLibraryInfo] = []
        validation_attempts = 0
        max_full_validation_candidates = 160
        ...
        def _enqueue_g55_validation_candidate(
            ...
            if len(group_ids) < 2:
                ...
                return
            high_phix_ratio = _g55_ratio(selected, _is_g55_high_phix_library)
            if high_phix_ratio > 0.30 + 1e-6:
                failure_counter["g55_high_phix_ratio_prequeue"] = failure_counter.get("g55_high_phix_ratio_prequeue", 0) + 1
                return
            customer_ratio = _g55_ratio(selected, _is_customer_library_candidate)
            if customer_ratio > 0.50 + 1e-6:
                failure_counter["g55_customer_ratio_prequeue"] = failure_counter.get("g55_customer_ratio_prequeue", 0) + 1
                return
            compatible, reason = _check_g55_cross_group_compatibility(selected)
        ...
        def _build_budgeted_g55_group_candidates(
            source_pool: List[EnhancedLibraryInfo],
        ) -> List[List[EnhancedLibraryInfo]]:
            """按基础组组合构造G55候选，入选过程中约束客户/high-phix比例。

            这里和1.1保持同一套逻辑，只由 mode_name 决定容量窗口：
            先给每个基础组放入至少一个可兼容文库，再按规则补齐容量。
            否则3.6阶段容易因为某个大组先填满窗口，导致预队列前退化为单基础组。
            """
            grouped_by_base: Dict[str, List[EnhancedLibraryInfo]] = {}
            for item in source_pool:
                group_id = _resolve_g55_base_group_id(item)
                if not group_id:
                    continue
                grouped_by_base.setdefault(group_id, []).append(item)
            if len(grouped_by_base) < 2:
                return []

            def group_data(group_id: str) -> float:
                return _g55_real_data_gb(grouped_by_base.get(group_id, []))

            def group_ratio(group_id: str, predicate) -> float:
                return _g55_ratio(grouped_by_base.get(group_id, []), predicate)

            group_order = sorted(
                grouped_by_base,
                key=lambda group_id: (
                    group_ratio(group_id, _is_customer_library_candidate),
                    group_ratio(group_id, _is_g55_high_phix_library),
                    -group_data(group_id),
                    group_id,
                ),
            )
            ...
            for i, first in enumerate(group_order):
                for second in group_order[i + 1 :]:
                    group_sets.append((first, second))
            for i, first in enumerate(group_order[:8]):
                for j, second in enumerate(group_order[i + 1 : 9], start=i + 1):
                    for third in group_order[j + 1 : 10]:
                        group_sets.append((first, second, third))

            def item_sort_key(item: EnhancedLibraryInfo) -> Tuple[Any, ...]:
                return (
                    1 if _is_customer_library_candidate(item) else 0,
                    1 if _is_g55_high_phix_library(item) else 0,
                    _get_delivery_priority_sort_value(item),
                    -_g55_lib_data(item),
                    -_count_library_index_pairs(item),
                    _safe_str(getattr(item, "origrec", ""), default=""),
            ...
            candidates_out: List[List[EnhancedLibraryInfo]] = []
            seen_signatures: Set[Tuple[str, ...]] = set()

            def _can_append(
                selected: List[EnhancedLibraryInfo],
                selected_index_pairs: List[Any],
                item: EnhancedLibraryInfo,
            ) -> Tuple[bool, Optional[Any], str]:
                if any(id(item) == id(selected_item) for selected_item in selected):
                    return False, None, "duplicate"
                trial = selected + [item]
                trial_total = _g55_real_data_gb(trial)
                if trial_total > max_allowed + 1e-6:
                    return False, None, "over_max"
                if _g55_ratio(trial, _is_customer_library_candidate) > 0.50 + 1e-6:
                    return False, None, "customer_ratio"
                if _g55_ratio(trial, _is_g55_high_phix_library) > 0.30 + 1e-6:
                    return False, None, "high_phix_ratio"
                buckets = {_g55_type_bucket(lib) for lib in trial}
                if "G58" in buckets and "G54" in buckets:
                    return False, None, "g58_g54_mix"
                has_conflict, item_index_pairs = _new_lib_has_latest_index_conflict_with_cache(
                    selected_index_pairs,
                    item,
                )
                if has_conflict:
                    return False, None, "index_conflict"
                return True, item_index_pairs, ""

            def _add_one_from_group(
                selected: List[EnhancedLibraryInfo],
                selected_index_pairs: List[Any],
                group_id: str,
            ) -> bool:
                for phase in ("clean", "all"):
                    for item in sorted(grouped_by_base.get(group_id, []), key=item_sort_key):
                        if phase == "clean" and (
                            _is_customer_library_candidate(item)
                            or _is_g55_high_phix_library(item)
                        ):
                            continue
                        ok, item_index_pairs, reason = _can_append(selected, selected_index_pairs, item)
                        if not ok:
                            failure_counter[f"g55_budgeted_seed_{reason}"] = failure_counter.get(f"g55_budgeted_seed_{reason}", 0) + 1
                            continue
                        selected.append(item)
                        selected_index_pairs.append(item_index_pairs)
                        return True
                return False

            for group_set in group_sets[:96]:
                group_pool: List[EnhancedLibraryInfo] = []
                for group_id in group_set:
                    group_pool.extend(grouped_by_base.get(group_id, []))
                selected: List[EnhancedLibraryInfo] = []
                selected_index_pairs: List[Any] = []
                seeded_groups: Set[str] = set()
                for group_id in group_set:
                    if _add_one_from_group(selected, selected_index_pairs, group_id):
                        seeded_groups.add(group_id)
                if len(seeded_groups) < 2:
                    failure_counter["g55_budgeted_seed_single_basic_group"] = failure_counter.get("g55_budgeted_seed_single_basic_group", 0) + 1
                    continue
                for phase in ("clean", "all"):
                    for item in sorted(group_pool, key=item_sort_key):
                        if phase == "clean" and (
                            _is_customer_library_candidate(item)
                            or _is_g55_high_phix_library(item)
                        ):
                        ...
                        ok, item_index_pairs, reason = _can_append(selected, selected_index_pairs, item)
                        if not ok:
                            failure_counter[f"g55_budgeted_fill_{reason}"] = failure_counter.get(f"g55_budgeted_fill_{reason}", 0) + 1
                            continue
                        selected.append(item)
                        selected_index_pairs.append(item_index_pairs)
                        if _g55_real_data_gb(selected) >= min_allowed - 1e-6:
                            break
                    if _g55_real_data_gb(selected) >= min_allowed - 1e-6:
                        break
                total = _g55_real_data_gb(selected)
                if total < min_allowed - 1e-6 or total > max_allowed + 1e-6:
                    continue
                base_groups = {_resolve_g55_base_group_id(item) for item in selected}
                base_groups.discard("")
                if len(base_groups) < 2:
                    continue
                compatible, reason = _check_g55_cross_group_compatibility(selected)
                if not compatible:
                    failure_counter[f"g55_budgeted_{reason or 'mix_incompatible'}"] = failure_counter.get(f"g55_budgeted_{reason or 'mix_incompatible'}", 0) + 1
                    continue
                signature = _g55_validation_signature(selected)
                if signature in seen_signatures:
                    continue
                seen_signatures.add(signature)
                candidates_out.append(selected)
        ...
        for sub_pool in pools:
                ...
                for selected in states:
                    ...
                    high_phix_ratio = (
                        sum(
                            float(getattr(item, "contract_data_raw", 0.0) or 0.0)
                            for item in trial
                            if _is_g55_high_phix_library(item)
                        )
                        / trial_total
                        if trial_total > 0
                        else 0.0
                    ...
                    customer_ratio = (
                        sum(
                            float(getattr(item, "contract_data_raw", 0.0) or 0.0)
                            for item in trial
                            if _is_customer_library_candidate(item)
                        )
                        / trial_total
                        if trial_total > 0
                        else 0.0
                    )
                    has_repairable_limit = (
                        ("G58" in buckets and "G54" in buckets)
                        or (
                            mode_name in {"1", "1.0", "1.1"}
                            and _mode_1_1_add_test_limited_data_gb(trial) > float(max_add_test_gb_per_lane or 0.0) + 1e-6
                        )
                        or high_phix_ratio > 0.30 + 1e-6
                        or customer_ratio > 0.50 + 1e-6
                    )
                    if has_repairable_limit:
                        continue
            ...
            validation_queue = sorted(
                validation_queue,
                key=lambda selected: (
                    _g55_ratio(selected, _is_customer_library_candidate),
                    _g55_ratio(selected, _is_g55_high_phix_library),
                    -sum(
                        1
                        for item in selected
                        if _is_mode_1_1_small_split_imbalance_priority_library(item)
            ...
            for selected in validation_queue:
                ...
                if reason in {"add_test_mixed_over_150", "g58_g54_mix_forbidden", "high_phix_ratio_over_30", "customer_ratio_over_50"}:
                    repaired_selected = _repair_g55_selection_mix_limits(
                        selected,
                        repair_pool=sub_pool,
                        min_allowed=min_allowed,
                        max_allowed=max_allowed,
                    ...
                    if repaired_selected:
                        ...
                        if repaired_lane is not None:
                            logger.info(
                                "{} 混合/加测限制定向修复成功: 原始{}个/{:.1f}G, 修复后{}个/{:.1f}G",
                                stage_label,
                                len(selected),
                                _g55_real_data_gb(selected),
                                len(repaired_selected),
                            ...
                            best_used = [lib for lib in repaired_selected]
                            break
                        failure_counter[f"mix_limit_repair_{repaired_reason}"] = failure_counter.get(f"mix_limit_repair_{repaired_reason}", 0) + 1
...
def _try_add_terminal_global_36t_mixed_lanes(
    ...
    if pool:
        ...
        incremental_added = 0
        while split_pack_added + incremental_added < max_lanes:
            remaining_pool = [lib for lib in pool if id(lib) not in used_ids]
            machine_type = resolve_terminal_lane_machine_type(remaining_pool)
            g53_g54_pool = [
                lib for lib in remaining_pool
...
def _try_add_terminal_remaining_10bp_nb_lanes(
    solution: Any,
    validator: Any,
    *,
    machine_type: MachineType = MachineType.NOVA_X_25B,
    ...
    """所有常规1.1/3.6流程结束后，用剩余自动10bp混客户/手工侧文库补NB Lane。"""
    unassigned = list(getattr(solution, "unassigned_libraries", []) or [])
    if not unassigned:
        return {"new_lanes": 0, "used_libraries": 0, "remaining_unassigned": 0}

    def _is_10bp(lib: EnhancedLibraryInfo) -> bool:
        ten_bp_data = getattr(lib, "ten_bp_data", None)
        if ten_bp_data is not None and ten_bp_data > 0:
            return True
        return bool(getattr(validator, "_is_10bp_index", lambda _: False)(getattr(lib, "index_seq", "") or ""))

    def _data(lib: EnhancedLibraryInfo) -> float:
        return float(getattr(lib, "contract_data_raw", 0.0) or lib.get_data_amount_gb() or 0.0)

    def _mix_side(lib: EnhancedLibraryInfo) -> str:
        return _normalize_wkjkhj_for_mix(
            getattr(lib, "wkjkhj", None)
            or getattr(lib, "_jkhj_raw", None)
            or ""
        )

    imbalance_data_limit = _resolve_special_library_data_limit(machine_type)

    def _is_terminal_nb_candidate(lib: EnhancedLibraryInfo) -> bool:
        return (
            not _is_ai_balance_library(lib)
            and not _is_split_library(lib)
            and not _is_split_rollback_unassigned_only(lib)
            and not _get_package_lane_number_from_library(lib)
            and not _is_truthy_flag(getattr(lib, "is_package_lane", None))
            and _data(lib) > 0
        )

    def _is_auto_10bp(lib: EnhancedLibraryInfo) -> bool:
        return _is_terminal_nb_candidate(lib) and _mix_side(lib) == "诺禾自动" and _is_10bp(lib)

    def _is_manual_customer_side(lib: EnhancedLibraryInfo) -> bool:
        return _is_terminal_nb_candidate(lib) and _mix_side(lib) in {"客户自建", "诺禾手工"}

    def _selected_data(libraries: List[EnhancedLibraryInfo], selector) -> float:
        return _total_lane_data([lib for lib in libraries if selector(lib)])

    def _business_mix_ok(libraries: List[EnhancedLibraryInfo]) -> bool:
        real_libraries = [lib for lib in libraries if not _is_ai_balance_library(lib)]
        if not real_libraries:
            return False
        total = _total_lane_data(real_libraries)
        if total <= 0:
            return False
        auto_10bp_data = _selected_data(real_libraries, _is_auto_10bp)
        manual_customer_data = _selected_data(real_libraries, _is_manual_customer_side)
        if auto_10bp_data <= 0 or manual_customer_data <= 0:
            return False
        # 混入后仍按LaneValidator规则收口：客户自建不能超过50%，且混合少数侧不低于5%。
        customer_data = _selected_data(real_libraries, lambda lib: _mix_side(lib) == "客户自建")
        if customer_data / total > 0.50 + 1e-9:
            return False
        if min(auto_10bp_data, manual_customer_data) / total < 0.05 - 1e-9:
            return False
        return True

    auto_10bp_pool = [lib for lib in unassigned if _is_auto_10bp(lib)]
    manual_customer_pool = [lib for lib in unassigned if _is_manual_customer_side(lib)]
    if not auto_10bp_pool or not manual_customer_pool:
        return {"new_lanes": 0, "used_libraries": 0, "remaining_unassigned": len(unassigned)}

    auto_10bp_pool.sort(
        key=lambda lib: (
            0 if _is_imbalance_library_candidate(lib) else 1,
            1 if _get_library_special_split_tokens(lib) else 0,
            _get_delivery_priority_sort_value(lib),
            -_data(lib),
            _safe_str(getattr(lib, "origrec", ""), default=""),
        )
    )
    manual_customer_pool.sort(
        key=lambda lib: (
            0 if _mix_side(lib) == "客户自建" else 1,
            1 if _is_10bp(lib) else 0,
            0 if _is_imbalance_library_candidate(lib) else 1,
            1 if _get_library_special_split_tokens(lib) else 0,
            _get_delivery_priority_sort_value(lib),
            -_data(lib),
    ...
    metadata = {
        ...
        "seq_mode": "1.1",
        "lcxms": "1.1",
        "dispatch_stage": "terminal_remaining_10bp_nb_mixed_lane",
        "is_mixed_10bp_nb_lane": True,
    }

    existing_serials: List[int] = []
    for lane in list(getattr(solution, "lane_assignments", []) or []):
        match = re.search(r"NB_[^_]+_(\d+)$", _safe_str(getattr(lane, "lane_id", ""), default=""))
        if match:
            try:
    ...
    def _can_append(
        ...
        _, trial_max_allowed = _resolve_lane_capacity_limits(
            trial,
            machine_type,
            lane_id=f"NB_{machine_type.value}_TMP",
            lane_metadata=metadata,
        )
        if total + lib_data > trial_max_allowed + 1e-6:
            return False, []
        if _shares_split_family_with_selected(selected, lib):
            return False, []
        projected_imbalance_data = _project_lane_special_data(selected, lib)
        projected_total = total + lib_data
        projected_imbalance_ratio = projected_imbalance_data / projected_total if projected_total > 0 else 0.0
        if projected_imbalance_data > imbalance_data_limit + SPECIAL_LIBRARY_LIMIT_EPSILON:
            return False, []
        if projected_imbalance_ratio > 0.35 + 1e-6:
            return False, []
        if _violates_mode_1_1_add_test_cap(trial, lane_metadata=metadata):
            return False, []
        ss_valid, _, _ = _validate_lane_special_split_rule(trial)
        if not ss_valid:
            return False, []
        idx_valid, lib_indices = _validate_new_lib_quick_with_result_cache(
            idx_validator=_MODULE_IDX_VALIDATOR,
        ...
        if not idx_valid:
            return False, []
        return True, lib_indices

    def _append_if_possible(
        selected: List[EnhancedLibraryInfo],
        selected_idx_cache: List[Any],
        ...
        if id(lib) in selected_ids:
            return total
        ok, lib_indices = _can_append(selected, selected_idx_cache, lib, total)
        if not ok:
            return total
        selected.append(lib)
        selected_idx_cache.append(lib_indices)
        selected_ids.add(id(lib))
        return total + _data(lib)

    def _build_lane_candidate() -> Tuple[List[EnhancedLibraryInfo], float]:
        selected: List[EnhancedLibraryInfo] = []
        selected_idx_cache: List[Any] = []
        selected_ids: Set[int] = set()
        total = 0.0
        available_auto = [lib for lib in auto_10bp_pool if id(lib) not in used_ids]
        available_manual_customer = [lib for lib in manual_customer_pool if id(lib) not in used_ids]
        candidate_pool = available_auto + available_manual_customer
        if not candidate_pool:
            return [], 0.0
        min_allowed, max_allowed = _resolve_lane_capacity_limits(
            candidate_pool,
        ...
        if _total_lane_data(candidate_pool) + 1e-6 < min_allowed:
            return [], 0.0

        def _current_capacity_bounds() -> Tuple[float, float]:
            if not selected:
        ...
        target_auto_data = min_allowed * 0.55
        while _selected_data(selected, _is_auto_10bp) < target_auto_data - 1e-6:
            before = total
            for lib in available_auto:
                total = _append_if_possible(selected, selected_idx_cache, selected_ids, lib, total)
                if total > before + 1e-9:
                    break
            if total <= before + 1e-9:
                break

        while total < min_allowed - 1e-6:
            customer_added = False
            for lib in available_manual_customer:
                if id(lib) in selected_ids:
                    continue
                trial_total = total + _data(lib)
                if trial_total > max_allowed + 1e-6:
                    continue
                trial_customer_data = _selected_data(selected, lambda x: _mix_side(x) == "客户自建")
                if _mix_side(lib) == "客户自建":
                    trial_customer_data += _data(lib)
                if trial_customer_data / trial_total > 0.50 + 1e-9:
                    continue
                before = total
                total = _append_if_possible(selected, selected_idx_cache, selected_ids, lib, total)
                if total > before + 1e-9:
        ...
        while True:
            current_min_allowed, current_max_allowed = _current_capacity_bounds()
            if total >= current_min_allowed - 1e-6:
                break
            if total >= current_max_allowed - 1e-6:
                break

            selected_total = max(total, 1e-9)
            auto_ratio = _selected_data(selected, _is_auto_10bp) / selected_total
            manual_customer_ratio = _selected_data(selected, _is_manual_customer_side) / selected_total
            preferred_pools = (
                [available_auto, available_manual_customer]
                if auto_ratio < 0.50 - 1e-9
                else [available_manual_customer, available_auto]
            )
            if manual_customer_ratio < 0.10 - 1e-9:
                preferred_pools = [available_manual_customer, available_auto]
            ...
            for pool in preferred_pools:
                for lib in pool:
                    if id(lib) in selected_ids:
                        continue
                    trial_total = total + _data(lib)
                    if trial_total > current_max_allowed + 1e-6:
                        continue
                    trial_customer_data = _selected_data(selected, lambda x: _mix_side(x) == "客户自建")
                    if _mix_side(lib) == "客户自建":
                        trial_customer_data += _data(lib)
                    if trial_customer_data / trial_total > 0.50 + 1e-9:
                        continue
                    trial_auto_10bp_data = _selected_data(selected, _is_auto_10bp)
                    if _is_auto_10bp(lib):
                        trial_auto_10bp_data += _data(lib)
                    if trial_auto_10bp_data / trial_total < 0.40 - 1e-9:
                        continue
                    before = total
                    total = _append_if_possible(selected, selected_idx_cache, selected_ids, lib, total)
                    if total > before + 1e-9:
        ...
        if total >= min_allowed - 1e-6 and not _business_mix_ok(selected):
            for lib in available_manual_customer:
                if id(lib) in selected_ids:
                    continue
                trial_total = total + _data(lib)
                ...
                before = total
                total = _append_if_possible(selected, selected_idx_cache, selected_ids, lib, total)
                if total > before + 1e-9 and _business_mix_ok(selected):
                    break

        final_min_allowed, final_max_allowed = _current_capacity_bounds()
        if (
            total < final_min_allowed - 1e-6
            or total > final_max_allowed + 1e-6
            or not _business_mix_ok(selected)
            or _count_lane_index_pairs(selected) < AI_LANE_MIN_INDEX_PAIRS
        ):
            return [], 0.0
        return selected, total

    for _ in range(max_lanes):
        lane_id = f"NB_{machine_type.value}_{next_serial:03d}"
        selected, total = _build_lane_candidate()
        if not selected:
            break
        ...
        if not getattr(result, "is_valid", False):
            logger.info(
                "终态剩余10bp混排NB补Lane未通过校验: lane={}, 使用{}个/{:.1f}G, auto10bp={:.1f}G, manual/customer={:.1f}G, errors={}",
                lane_id,
                len(selected),
                total,
                _selected_data(selected, _is_auto_10bp),
                _selected_data(selected, _is_manual_customer_side),
                [getattr(err, "message", str(err)) for err in list(getattr(result, "errors", []) or [])[:5]],
            )
            break
        ...
        added_lanes.append(lane)
        logger.info(
            "终态剩余10bp混排NB补Lane成功: lane={}, 使用{}个/{:.1f}G, auto10bp={:.1f}G, manual/customer={:.1f}G",
            lane.lane_id,
            len(selected),
            total,
            _selected_data(selected, _is_auto_10bp),
    ...
    return {
        "new_lanes": len(added_lanes),
        "used_libraries": len(used_ids),
...
def _pick_special_split_removals(
    libraries: List[EnhancedLibraryInfo],
    *,
    prefer_delivery_priority: bool = False,
) -> List[EnhancedLibraryInfo]:
    ...
    lane_sample_types = _get_lane_sample_types(remaining)
    if lane_sample_types & LANE_LOADING_HD_VISIUM_TYPES and lane_sample_types & LANE_LOADING_COMBO_GROUP_B:
        hd_items = [
            lib for lib in remaining
            if _library_sample_type_matches_rule(lib, LANE_LOADING_HD_VISIUM_TYPES)
        ]
        group_b_items = [
            lib for lib in remaining
            if _library_sample_type_matches_rule(lib, LANE_LOADING_COMBO_GROUP_B)
        ]
        hd_data = sum(float(getattr(lib, "contract_data_raw", 0.0) or 0.0) for lib in hd_items)
        group_b_data = sum(float(getattr(lib, "contract_data_raw", 0.0) or 0.0) for lib in group_b_items)
        keep_hd = hd_data >= group_b_data
        if prefer_delivery_priority and hd_items and group_b_items:
            hd_delivery = _get_libraries_delivery_priority_sort_key(hd_items)
            group_b_delivery = _get_libraries_delivery_priority_sort_key(group_b_items)
            if hd_delivery < group_b_delivery:
                keep_hd = True
            elif group_b_delivery < hd_delivery:
                keep_hd = False
        removal_pool = group_b_items if keep_hd else hd_items
        return sorted(
            removal_pool,
            key=lambda lib: (
                _get_delivery_priority_remove_sort_value(lib)
                if prefer_delivery_priority
                else 0.0,
                float(getattr(lib, "contract_data_raw", 0.0) or 0.0),
                _get_library_identity_key(lib),
            ),
    ...
    # A/B混排：非包Lane修复可优先保留交付更早的一侧，打平再保留数据量更大的组。
    if has_a and has_b:
        keep_mode = "A" if data_by_mode["A"] >= data_by_mode["B"] else "B"
        if prefer_delivery_priority:
            delivery_by_mode: Dict[str, Tuple[float, float, float]] = {
                mode: _get_libraries_delivery_priority_sort_key(
                    [lib for lib, lib_mode in mode_records if lib_mode == mode]
                )
                for mode in ("A", "B")
            }
            if delivery_by_mode["A"] < delivery_by_mode["B"]:
                keep_mode = "A"
            elif delivery_by_mode["B"] < delivery_by_mode["A"]:
                keep_mode = "B"
        to_remove.extend(
            sorted(
                [
                    lib for lib in remaining
                    if _classify_library_special_split_mode(lib) in {"A", "B"}
                    and _classify_library_special_split_mode(lib) != keep_mode
                ],
                key=lambda lib: (
                    _get_delivery_priority_remove_sort_value(lib)
                    if prefer_delivery_priority
...
def _auto_fix_lane_for_special_splits(
    ...
    prefer_delivery_priority = not _is_package_lane_assignment(lane)
    removals = _pick_special_split_removals(
        list(lane.libraries),
        prefer_delivery_priority=prefer_delivery_priority,
    )
    if not removals:
    ...
    # 先从未分配池补齐
    if prefer_delivery_priority:
        unassigned_candidates = sorted(
            list(unassigned_pool),
            key=lambda x: (
                _get_delivery_priority_sort_value(x),
                -float(getattr(x, "contract_data_raw", 0.0) or 0.0),
                _get_library_identity_key(x),
            ),
        )
    else:
        unassigned_candidates = sorted(
            list(unassigned_pool),
            key=lambda x: float(getattr(x, "contract_data_raw", 0.0) or 0.0),
            reverse=True,
        )
    for lib in unassigned_candidates:
    ...
    if lane.total_data_gb < min_allowed:
        for donor_lane in all_lanes:
            if donor_lane is lane:
                continue
            if prefer_delivery_priority and not _is_package_lane_assignment(donor_lane):
                donor_candidates = sorted(
                    list(donor_lane.libraries),
                    key=lambda x: (
                        _get_delivery_priority_remove_sort_value(x),
                        -float(getattr(x, "contract_data_raw", 0.0) or 0.0),
                        _get_library_identity_key(x),
                    ),
                )
...
def _collect_prediction_rows(
    ...
    for lane_sorter, lane in enumerate(lanes, start=1):
        ...
        if is_mode_1_1_round2_lane:
            ...
            lane_output_seq_mode = (
                _output_sequencing_mode_from_rule_code(lane_rule_code)
                or lane_sequencing_mode
                or "3.6T-NEW"
            )
            lane_selected_seq_mode = lane_output_seq_mode
            lane_sequencing_mode = lane_output_seq_mode
...
def _build_detail_output(
    ...
    def _default_output_seq_mode_for_detail_row(row: pd.Series) -> str:
        """按规则矩阵中的实验室配置推断3.6 Lane最终输出模式。"""
        lab_text = _normalize_text_for_match(
            row.get("wkdept")
            or row.get("dept")
            or row.get("lab_name")
            or row.get("LAB_NAME")
        ...
        matched_process_code: Optional[int] = None
        scheduling_config = get_scheduling_config()
        for mapping in scheduling_config._rule_matrix_config.get("lab_process_mappings", []):
            ...
            if lab_text and any(lab_name and lab_name in lab_text for lab_name in lab_names):
                try:
                    matched_process_code = int(process_code)
                except (TypeError, ValueError):
                    matched_process_code = None
                break

        if matched_process_code is not None:
            for profile in scheduling_config._rule_matrix_config.get("lane_rule_profiles", []):
                rule_code = _safe_str(profile.get("rule_code"), default="")
                if not (
                    rule_code.endswith("_special_combo_36t")
                    or _is_standard_pe150_25b_capacity_rule(rule_code)
                ):
                    continue
                process_codes = set(profile.get("process_codes") or set())
                if matched_process_code in process_codes:
                    return _safe_str(
                        profile.get("output_sequencing_mode") or profile.get("sequencing_mode"),
                        default="3.6T",
                    )
    ...
    fallback_lcxms_mask = lane_assigned_mask & still_missing_mask
    if fallback_lcxms_mask.any():
        merged.loc[fallback_lcxms_mask, "lcxms"] = merged.loc[fallback_lcxms_mask].apply(
            _default_output_seq_mode_for_detail_row,
            axis=1,
        )
...
def arrange_library(
    ...
    normal_libs = _sort_remaining_for_scattered_mix_lane(normal_libs)
    ...
    if normal_libs:
        ...
        single_group_imbalance_lanes, normal_libs, _ = _keep_only_mode_1_1_prebuilt_lanes(
    ...
    if mode_1_1_config:
        ...
        singapore_36t_libs, pool_1_1_all = _partition_singapore_libraries_for_36t(pool_1_1_all)
        if singapore_36t_libs:
            normal_libs_for_36t.extend(singapore_36t_libs)
            logger.info(
                "新加坡文库强制回流3.6T-NEW: {}个，直接跳过1.1首轮与尾货抽取",
                len(singapore_36t_libs),
            )
    ...
    if normal_libs:
        ...
        trailing_dedicated_imbalance_lanes, normal_libs, _ = _keep_only_mode_1_1_prebuilt_lanes(
    ...
    pre_36t_customer_lanes: List[LaneAssignment] = []
    if normal_libs:
        pre_36t_customer_lanes, normal_libs, pre_36t_customer_stats = (
            _preextract_36t_pure_customer_lanes(
                normal_libs,
                validator=stage_validator,
                max_lanes=8,
            )
        )
        if pre_36t_customer_lanes:
            logger.info(
                "步骤2前置3.6客户纯Lane抽取完成: 新增Lane={}, 使用文库={}, 剩余普通排机文库={}",
                int(pre_36t_customer_stats.get("pure_customer_lanes", 0)),
                int(pre_36t_customer_stats.get("used_libraries", 0)),
                len(normal_libs),
            )
    ...
    terminal_10bp_nb_stats = _try_add_terminal_remaining_10bp_nb_lanes(
        solution,
        final_cleanup_validator,
    )
    if terminal_10bp_nb_stats["new_lanes"] > 0:
        logger.info(
            "最终剩余10bp混排NB补Lane完成: 新增Lane={}，使用文库={}，剩余未分配={}",
            terminal_10bp_nb_stats["new_lanes"],
            terminal_10bp_nb_stats["used_libraries"],
            terminal_10bp_nb_stats["remaining_unassigned"],
        )
        terminal_10bp_cleanup_stats = _final_non_package_validation_cleanup(solution, final_cleanup_validator)
        if terminal_10bp_cleanup_stats["removed_lanes"] > 0:
            logger.warning(
                "最终剩余10bp混排NB补Lane后二次总复核: 淘汰{}条不合规Lane，回收{}个文库",
                terminal_10bp_cleanup_stats["removed_lanes"],
                terminal_10bp_cleanup_stats["recovered_libs"],
            )
    else:
        logger.info(
